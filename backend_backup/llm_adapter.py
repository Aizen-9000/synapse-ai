...
class LLMAdapter:
    def __init__(self):
        ...
        self.model_secondary = os.getenv("GROQ_MODEL_SECONDARY", "llama-3.3-70b-versatile")
        self.base_url = "https://api.groq.com/openai/v1/chat/completions"

    async def generate(self, prompt: str, max_tokens: int = 512, temperature: float = 0.7, model: str = None):
        """
        Generate text using Groq AI API.
        - `model` param: optional, if not passed, uses default model
        """
        if not self.api_key:
